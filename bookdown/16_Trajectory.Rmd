...
ArchR can also handle trajectories derived in [Monocle3](https://cole-trapnell-lab.github.io/monocle3/). This is handled by the `getMonocleTrajectories()` and `addMonocleTrajectory()` functions. Before diving in, it is worth noting that much of the infrastructure that ArchR uses for storing and visualizing trajectories is based on the assumption that the trajectory is linear and the trajectories determined by Monocle3 are not linear. Because of this, we have to treat the resulting Monocle3 trajectory as a collection of individual trajectories coming from a single cellular group.

Though this is slightly counter to the typical ArchR workflow, we first run `getMonocleTrajectories()` to retrieve the trajectory from Monocle3. Our tutorial dataset isnt very deeply branched so here we will simultaneously model the myeloid and lymphoid trajectories that we did in isolation in the previous sections. In this function call, `principalGroup` indicates the apex of the trajectory from which all branches emanate and `useGroups` tells Monocle3 all of the groups that should be mapped on the trajectory. 
...
You'll notice that during the above function call, ArchR tells you that the trajectory results were plotted to a PDF called `Monocole3/Plot-Results-Monocle_Progenitor.pdf` which you can reference to see the trajectory results.

After we have retrieved this trajectory object from Monocle3 (called a "cell data set" or CDS), we need to add this information to our `ArchRProject` object. To do this, we use `addMonocleTrajectory()` and pass it the output of `getMonocleTrajectories(`. However, as mentioned above, the Monocle trajectory contains multiple branches so we need to add each branch individually as its own trajectory. First, we will add the myeloid trajectory, by specifying the myeloid cell groupings to `useGroups`.
...
Alternatively, a more straightforward approach would be to create each "branch" (myeloid / lymphoid) as a separate call to `getMonocleTrajectories()`, providing only the relevant groups to `useGroups`. For certain applications, this may be a cleaner implementation so it is worth thinking about the best way to do this for your particular biological question. To enable downstream comparison of ArchR, Monocle3, and Slingshot trajectories in a subsequent secion of this book, we will do this for the lymphoid trajectory below.
...
If you look closely, you'll notice slight differences between this lymphoid trajectory identified in isolation compared to the lymphoid branch of the larger joint monocle trajectory calculated at the beginning of this section.
