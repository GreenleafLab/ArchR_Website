...
Alternatively, we could create a single trajectory encompassing both the myeloid and lymphoid brances with a single call to `getMonocleTrajectories()`, providing all of the relevant groups to `useGroups`. For certain applications, this may be a useful implementation so it is worth thinking about the best way to do this for your particular biological question.
